...
public class Paging {
	...
	private int section;				// section이 0이면 1~10, 1이면 11~20, 2이면 21~30
	...
	public Paging(int page, int boardCount) {		// 생성자 (요청받은 페이지, 총 게시글 수)
		...
		// 전체 게시글 수를 페이지 당 출력개수로 나누고, 나머지가 있으면 1페이지를 더한다.
		
		end = end >= pageCount ? pageCount : end;
		// 마지막 페이지 번호와 마지막 섹션번호를 비교하여 작은 값을 선택한다. 
		// (41 ~ 50 구간이지만 실제로는 46페이지까지만 있을 경우)
		...
		// 현재 보고 있는 페이지가 13이고, begin은 11이면 perPage는 10이므로, 이전 섹션이 존재한다는 의미임
		
		next = lastSection > section;
		// 마지막 섹션이 현재 섹션보다 더 큰 값이면 [다음]을 출력한다.
