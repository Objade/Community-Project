...
<form method="POST">
	...
	<p>
		...
		<input type="button" name="dupCheck" value="중복 체크"><br>
...
<script>
	// 스프링의 특정 주소로 요청하여 결과를 받고, 결과에 따라서 다른 메시지를 만들어내는 자바 스크립트 (AJAX)
	...
	dupCheck.onclick = function(event) {
		...
		.then(text => {							// 문자열을 이용하여 다음 코드를 진행합니다
			...
			const code = text.split(':')[0]		// 1 : 사용 중, 0 : 중복 없음
...
<script>
	// 약관을 90퍼센트 이상 읽어야지만 약관 동의가 활성화 되도록 함
	// 스프링과 상관없이 단독으로 실행되는 자바 스크립트
